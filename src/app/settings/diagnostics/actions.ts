...
/**
 * Diagnostics Server Actions — the super-admin-only live debugging and
 * field-discovery surface behind /settings/diagnostics.
 *
 * These lived in settings/instances/actions.ts until this file was created.
 * They were moved for CAPABILITY OWNERSHIP, not to change behaviour: that
 * file mixed two different security capabilities in one place, which made
 * directory-based action ownership impossible to reason about and blocked a
 * clean MODULES-derived structural guard.
 *
 * DIAGNOSTICS_MODULE is deliberately NOT in module-nav's MODULES: this is a
 * non-org-toggleable, super-admin-only capability. So nothing here is gated
 * on requireModuleAccess(INSTANCES_MODULE.href) — coupling a deliberately
 * non-toggleable super-admin surface to an org visibility toggle would be
 * wrong, not stricter. Each action instead stands on its own
 * requirePrivilegedSuperAdmin boundary; the route layout's requireSuperAdmin
 * is navigation protection only.
 */
...
import {
  findProductWithBom,
  probeWorkCentrePaths,
  findCustomerAndSupplierExamples,
  checkCustomerReferenceFields,
  checkSupplierReferenceFields,
  findCustomerRawByName,
  findAccountsByCodes,
  checkSaleStatuses,
  findFinishedGoodsExample,
  surveyFinishedGoodsFields,
  surveyCostBasisFields,
  surveyProductionBomFields,
  surveyProductionBomForSkus,
  surveyProductionOrderDetail,
  surveyProductionOrderRoutingTasks,
  surveyProductionOrderOperationStatus,
  surveyProductionRun,
  surveyProductionOrderStatuses,
  surveyPurchaseDetailFields,
  surveyProductAvailabilityFields,
  surveySaleFulfillmentFields,
  surveyBackorderEtaFields,
  testSaleShipByWriteBack,
  testProductSupplierLink,
  surveyProductSupplierOptionsFields,
  findProductSupplierOptionsExample,
  testCreatePurchaseOrder,
  checkProductAvailabilityForSkus,
  checkProductSupplierOptionsForUnparsedFields,
  probeUpdatedSinceFiltering,
} from "@/cin7/debug";
...
export interface DiagnosticInstancesResult {
  ok: boolean;
  error?: string;
...
/**
 * The instance list this page needs, and only that: id, name and account id
 * are the three fields the Diagnostics UI renders. Deliberately NOT the
 * Instances-settings DTO — keyLast4, the fulfilment view floor and the
 * created-at timestamp are Instances-settings metadata with no diagnostic
 * use, so they are not selected here at all. No secret is ever returned.
 *
 * Independent of Instances' own listInstances by design: this is a different
 * capability with a different guard. Following the precedent recorded in
 * settings/members/actions.ts, the two small queries are kept as mirrored
 * implementations rather than extracted into a new cross-capability helper.
 */
export async function listDiagnosticInstancesAction(): Promise<DiagnosticInstancesResult> {
  try {
    // The route layout's requireSuperAdmin is navigation protection only — a
    // Server Action must establish its own boundary (CCT-ADR-0010).
    await requirePrivilegedSuperAdmin("view diagnostic instances");
    ...
    return {
      ok: true,
      instances: (data ?? []).map((row) => ({ id: row.id, name: row.name, accountId: row.account_id })),
    };
  } catch (e) {
...
/**
 * Mirrored from settings/instances/actions.ts rather than extracted into a
 * shared helper: see settings/members/actions.ts for the precedent this
 * follows — a previous production failure after extracting a brand-new
 * helper whose only callers crossed two capabilities. Behaviour here is
 * identical to the original, deliberately including the absence of an
 * `active` check (a diagnostic must still be runnable against an inactive
 * instance, unlike @/cin7/load-credentials' loadCin7Credentials).
 */
...
/** Mirrored from settings/instances/actions.ts — same reasoning as loadInstanceCreds above. */
