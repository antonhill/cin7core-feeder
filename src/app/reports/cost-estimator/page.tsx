...
/**
 * Production BOM costing isn't built yet — this codebase has never read
 * anything beyond a Production BOM's `.Version` back from Cin7's live API
 * (see production-bom.ts's findProductionBomVersion), and whether Resources
 * (labor/machine) carry any cost data at all, or only material Components
 * do, is still being confirmed live (Settings > Cin7 Instances' Production
 * BOM diagnostics). Shown honestly as not-yet-available rather than a
 * silently empty/broken tool.
 */
function ProductionBomsPlaceholder() {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <p className="font-medium text-slate-900">Production BOMs</p>
      <p className="mt-2 text-sm text-slate-500">
        Cost estimation for Production BOMs (Operations, Components, and
        Resources) isn&rsquo;t available yet — still being verified against
        Cin7&rsquo;s live API, specifically whether Resources (labor/machine)
        carry any cost data at all, or only material Components do. This section
        will light up once that&rsquo;s confirmed.
      </p>
    </section>
  );
}
