...
function makeFakeDb(opts: FakeDbOptions) {
  ...
  const db = {
    from: (table: string) => {
      ...
      if (table === "purchase_receipt_lines") {
        return {
          delete: () => {
...
describe("syncInstancePurchases — list phase", () => {
  it("skips purchases that have never received anything", async () => {
...
describe("syncInstancePurchases — detail phase", () => {
  ...
  it("handles a purchase detail response with no receipt lines", async () => {
    vi.mocked(fetchPurchaseDetail).mockResolvedValueOnce({ source: "purchase", receiptLines: [] });
    const { db, calls } = makeFakeDb({ existingPurchases: [], pendingPurchases: [{ cin7_purchase_id: "po-1" }] });
