-- P5.2 (LBL brief): Backorder "no open PO" filter. The line-level data
-- already distinguishes a backordered line with a linked open PO (+ETA)
-- from one with no open PO — report_order_fulfillment_lines'
-- backorder_po_number/backorder_eta/backorder_po_outstanding_qty, already
-- rendering in Order Fulfillment's expanded row today. This migration adds
-- the ORDER-level aggregate the client asked for: does this order have
-- ≥1 backordered line lacking a linked open PO (the actionable procurement
-- list), and separately, ≥1 backordered line that DOES have one. The two
-- aren't mutually exclusive — a mixed order (one line covered by a PO,
-- another not) qualifies for both, which is intentional: each is its own
-- independent "does at least one line match" filter, not a 3-way partition.
--
-- Client specifically asked whether the EXISTING "Backorders" filter
-- (Order Fulfillment page — total_backorder_qty > 0) is status- or
-- linkage-based: it's status-based (a raw quantity check). This migration
-- doesn't change that filter — it adds a second, explicitly linkage-based
-- one alongside it, with its own tooltip spelling out the difference.
