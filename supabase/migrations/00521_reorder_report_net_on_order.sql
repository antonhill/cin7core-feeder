-- Fixes report_reorder (0048): needs_reorder and the "Stockout risk" status
-- branch compared on_hand alone against the threshold/zero, never netting
-- off on_order — the exact same gap Purchase Planner had (fixed in
-- src/reports/supplier-planner/build.ts, 2026-07-27) before it was found
-- and fixed there first. A product with a big shipment already inbound got
-- flagged "needs reorder"/"Stockout risk" purely because on-hand alone had
-- dipped low, even though on_order already covered it. reorder_threshold
-- and weeks_of_cover are untouched — different metrics, not part of this
-- specific gap (weeks_of_cover in particular is a user-facing "how long
-- until empty" figure that deliberately still reflects on-hand only).
...
returns table (
  ...
  select
    ...
    (c.on_hand + c.on_order) <= (c.total_out * (1 + p_buffer_percent / 100.0)) as needs_reorder,
    case
    ...
    case
      when (c.on_hand + c.on_order) <= 0 and c.total_out > 0 then 'Stockout risk'
