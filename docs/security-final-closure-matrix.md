...
**STATUS: PROVEN CLOSED (2026-08-18).**
- **Implementation:** `src/app/settings/instances/actions.ts` — new shared `logPrivilegedWrite` helper wraps `logActivity` with a mandatory `outcome: "success" | "failed" | "ambiguous"` field. All 4 write-capable diagnostic actions now log on every path: success, definite failure, and (for `debugTestSaleShipByWriteBack`, `debugTestProductSupplierLink`, `debugTestCreatePurchaseOrder`) an honest `"ambiguous"` outcome when the underlying `Cin7ApiError.ambiguous` flag is set. `debugPushOneCustomerAndSupplier` logs one combined record tracking each of the customer/supplier sub-push's own outcome. `upsertInstance` (both create and update paths) and `deleteInstance` now log success/failure, deliberately never including the raw or rotated Application Key in the logged `detail` — only whether a key rotation happened (boolean).
- **Migration:** none.
- **Tests:** same `src/app/settings/instances/__tests__/actions.test.ts` — behavioral tests for all 6 write paths: `upsertInstance` create success/failure, update success/failure (asserts `rotatedKey` flag present, raw key string absent from the logged payload via a `JSON.stringify` substring check); `deleteInstance` success/failure; `debugTestCreatePurchaseOrder` success/ambiguous/definite-failure outcomes, and confirms a pre-guard auth rejection logs nothing at all; `debugPushOneCustomerAndSupplier` combined-outcome logging for both-success and one-ambiguous cases.
- **Live evidence:** N/A.
- **CI:** PR #57.
...
**STATUS: PROVEN CLOSED (2026-08-18) — Anton's decision D2 (delete all six) implemented.**
- **Implementation:** the exported `POST` handler was removed entirely from all 6 route files. `GET` (the real Vercel Cron entry point) and `maxDuration` are unchanged. No replacement Server Actions were built for `sync-purchases`/`sync-assembly-builds` (explicitly out of scope per D2 — "that is a separate product requirement" if ever needed).
- **Migration:** none (pure route-file change).
- **Tests:** `src/test/__tests__/internal-tenant-scoping-route.test.ts` (new) — confirms all 6 routes' `POST` export is gone while `GET` remains; a generic scan proving no route handler anywhere in the repo combines a shared internal secret + a request-body/query tenant identifier + a service-role client without a session guard (the exact shape this blocker was); confirms known-safe routes (`webhooks/lemonsqueezy`, `delete-expired-trials`, `notify-ship-by-changes`) correctly don't trip the scan.
...
- **CI:** PR #57.
- **Permanent regression guard:** the generic internal-tenant-scoping scan above — a future route reintroducing this exact pattern (shared secret + body-supplied org id + service-role client, no session guard) fails CI immediately, without needing to specifically know about `/api/sync*` again.
...
- [x] Every closure test above passes locally (`tsc`, `eslint`, `vitest run` — 1260/1260 across 132 files, `next build`) — see PR #57 for the full CI pipeline result (install/lint/typecheck/test/build, dependency scan, secret scan, migration+RLS test matrix).
...
### Verification output (final closure)
...
CI run (all jobs green): PR [#57](https://github.com/antonhill/cin7core-feeder/pull/57) — [run #32126551216](https://github.com/antonhill/cin7core-feeder/actions/runs/32126551216)
