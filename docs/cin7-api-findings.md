...
**Confirmed live the same day: this gap is specific to Chart of Accounts, not the other pre-flight fields.** When this account error surfaced on its own (no Location/SalesRepresentative errors alongside it), that itself confirms `/ref/location` and `/me/contacts` are complete and final checks — they're pure Cin7-internal reference books with no external accounting system in the loop, so once they pass pre-flight they won't resurface on a later push for the same row. Only Chart-of-Accounts fields (AccountReceivable/SaleAccount/AccountPayable) carry this residual Xero/QuickBooks-sync risk that a Cin7-only exists-check can't rule out — closing it for good would need actual Xero/QuickBooks API access to check account-sync/mapping status ahead of time, which is a real scope decision (Anton's call, deferred for now — 2026-07-05: "just document the limit and move on").
